...
class Util1
{
    ...
    private static function initLanguageMap($language = '')
    {
        ...
        if (isset(self::$maps[$language]) && is_array(self::$maps[$language])) {
            // Move this map to end. This means it will have priority over sample
    ...
    public static function var_dump_plain($var, $expLevel, $depth = 0, $done = array())
    {
        ...
        if (is_bool($var)) {
            ...
            self::$hasArray = true;
            $uuid = 'includes-php-' . uniqid() . mt_rand(1, 1000000);
     ...
     * @param   integer $decimals The number of decimal points to includes
     * @return  string
     */
    ...
    public static function nocache_headers()
    {
        ...
        if (!headers_sent()) {
            ...
            header('Cache-Control: no-view_cache, must-revalidate, max-age=0');
     ...
     *                                      O and 0, l and 1, etc)
     * @param   boolean $include_symbols    Whether or not to includes
    ...
    {
        // *grumbles* stupid PHP type route
