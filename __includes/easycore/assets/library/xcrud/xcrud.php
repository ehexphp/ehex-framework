...
function Xcrud_instance(){
    ...
    $xcrud->connection(Config1::DB_USER, Config1::DB_PASSWORD, Config1::DB_NAME, Config1::DB_HOST,'utf8');


    $xcrud->email_from = Config1::MAIL_EMAIL;
    $xcrud->email_from_name = Config1::APP_TITLE;
...
// Ehex Patch
//$this->runIfUrl($this->before_update['callable']);
...
class Xcrud {
    ...
    protected $table_info = null; // fields information from __FILE_BASE
    ...
    public function send_external($path, $data = array(), $method = 'includes', $mode = 'all', $where_field = '', $where_val =
        '')
    {
        if ($where_field)
        {
    ...
    /** returns current views into main container */
    ...
    /**
     * Xamtax Ehex (EX) Support Method.
     * Since All Ehex Model Class are not Visible in XCURD, I decided to
     * be calling api link with curl on method instead. This api key will include primary id in the response
     */
    protected function runIfUrl($link, $parameter = []){
        if(Url1::isValidUrl($link)) Console1::log( Url1::cURL_lite(Url1::replaceParameterAndGetUrl(array_merge(['id'=>$this->primary_val], $parameter) , [], $link)) );
    }
    ...
    {
        $del = false;
        if ($this->table_ro) return self::error('Forbidden');
        if ($this->before_remove) {

            // Ehex Patch
            $this->runIfUrl($this->before_remove['callable']);
        ...
        if ($this->replace_remove)
        {
            ...
            // Ehex Patch
            $this->runIfUrl($this->replace_remove['callable']);
        ...
        {
            // Ehex Patch
            $this->runIfUrl($this->after_remove['callable']);
    ...
    protected function _save()
    {
        ...
        if (!$this->primary_val)
        {
            ...
            {
                // Ehex Patch
                $this->runIfUrl($this->before_insert['callable']);
            ...
            {
                // Ehex Patch
                $this->runIfUrl($this->replace_insert['callable']);
            ...
            {
                // Ehex Patch
                $this->runIfUrl($this->after_insert['callable']);
        ...
        else
        {
            ...
            {
                // Ehex Patch
                $this->runIfUrl($this->after_update['callable']);
    ...
    /** 'select' subquery for grid views */
    ...
    /** creates subselect subquery for grid views */
    ...
    /** 'select' subquery part for edit/details views */
    ...
    /** renders details views template */
    protected function _render_details($mode)
    {
        ...
        */
        ...
        ob_end_clean();
        /*if ($this->inner_table_instance && ($mode == 'views' or $mode == 'edit')) // restoring nested objects
        {
        foreach ($this->inner_table_instance as $inst_name => $field)
        {
        if (isset($this->result_row[$field]))
        ...
        if ($mode == 'views' && Xcrud_config::$nested_readonly_on_view)
        {
        $instance->table_ro = true;
        }
        else
    ...
    public static function check_url($url, $scr_url = false){
        /**
         * Xamtax Edit [ assign a site relative path ]
         */
        $xcrud_ehex_path = Page1::getEhexCoreAssetsPath(). '/library/xcrud';

        if (!$url && !$scr_url)  return false;
        $url = rtrim($url, '/');
        ...
        //dd('-----COMLETED-----', $url);
        //dd($_SERVER['DOCUMENT_ROOT'], dirname(__file__), $doc_root, $file_dir);

        
        
        
        $url = trim(str_replace('\\', '/', str_replace(array('/public_html', '/private_html'), '', $url)), '/');
        $url = String1::replace($url, $doc_root, '');
       //dd($doc_root, $file_dir, $url);
        return $url;
    }
    ...
    /** renders action button for details views */
