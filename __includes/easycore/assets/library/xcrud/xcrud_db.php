        ...
        if (!isset(self::$_instance[$instance_name]) or null === self::$_instance[$instance_name])
        {
            if (!is_array($params))
            {
                ...
                global $GLOBALS;
                 ...
                 //list($dbuser, $dbpass, $dbname, $dbhost, $dbencoding) =  [$GLOBALS['config']['db_user'], $GLOBALS['config']['db_password'], $GLOBALS['config']['db_name'], $GLOBALS['config']['db_host'], 'utf8']; //xcrud_get_db_connection();
