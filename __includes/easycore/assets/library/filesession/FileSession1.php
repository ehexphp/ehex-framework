...
// https://github.com/bendahrooge/PHP-Simple-Database
...
class FileSession1 {
    ...
    protected $dev_mode;
    public function __construct($database_name = 'default', $path = 'file_session/', $dev_mode = true)
    {
        $this->path = $path;
    ...
    public function set($key, $value, $expire = 0)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    $this->handleError('Failed to create new __FILE_BASE, read and write permissions are required', true);
                    return false;
                }
            }
        }
    ...
    public function del($key){ return $this->fileOps($key, 'delete'); }

    public function get($key, $defaultValue = null){
        //return $this->fileOps($key, 'return');
        return $this->has($key)? $this->fileOps($key, 'return'): $defaultValue;
    }

    public function search($key){ return $this->fileOps($key, 'search');}

    public function has($key){return $this->search($key); }

    public function getAll($include_meta_data = false){
        if($include_meta_data == true) {return array_shift(json_decode(file_get_contents($this->datatbase)));}
        else {return json_decode(file_get_contents($this->database));}
    }
