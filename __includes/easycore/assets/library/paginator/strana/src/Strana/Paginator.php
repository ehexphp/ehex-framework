...
class Paginator
{
     ...
     */
    public function make($records = [], $paginationTemplateClass = \DefaultPaginationTemplate::class,  $adapter = null, $config = array(), $pageKeyName = 'page')
    {
        $config = array_merge($this->getConfig(), $config, ['pageKeyName'=>$pageKeyName]);
        ...
        $links = $linkCreator->createLinks($recordSet->total(), $paginationTemplateClass);
    ...
    protected function generate($records, ConfigHelper $configHelper)
    {
        ...
        $recordSet = new RecordSet($slicedRecords,  $total, (isset($_GET[$this->getConfig()['pageKeyName']])? $_GET[$this->getConfig()['pageKeyName']]: 1) );
        return $recordSet;
    }
