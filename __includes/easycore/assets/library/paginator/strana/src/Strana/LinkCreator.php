...
class LinkCreator {
     ...
     * @return string
     */
    function createLinks($totalRecords, $paginationTemplateClass = \DefaultPaginationTemplate::class){
        ...
        // previous
        $prevLiClass = $paginationTemplateClass::$previousClass;
        $prevLinkHref = 'javascript:void(0)'; //$paginationTemplateClass::$noLinkHref;
        if ($currentPage == 1)  $prevLiClass = $paginationTemplateClass::$disableClass;
        else  $prevLinkHref = $this->buildQueryString($currentPage - 1);

        // next
        $nextLiClass =  $paginationTemplateClass::$nextClass;
        $nextLinkHref = 'javascript:void(0)'; //$paginationTemplateClass::$noLinkHref;
        if ($currentPage == $totalPages) $nextLiClass = $paginationTemplateClass::$disableClass;
        else $nextLinkHref = $this->buildQueryString($currentPage + 1);


        $output = $paginationTemplateClass::getContainerOpen();
        $output .= $paginationTemplateClass::getPreviousItem($prevLiClass, $prevLinkHref);
        foreach($pages as $page) {
            $currentClass = $page == $currentPage ? $paginationTemplateClass::$activeClass : '';
            $output .= $paginationTemplateClass::getActiveItem($currentClass, $this->buildQueryString($page), $page);
        }
        $output .= $paginationTemplateClass::getNextItem($nextLiClass, $nextLinkHref);
        $output .= $paginationTemplateClass::getContainerClose();
