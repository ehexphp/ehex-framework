<?php
    $login_key = 'a13vfjnk324kjb235bcdea13vfjnk324kjb235bcdea13vfjnk324kjb235bcdea13vfjnk324kjb235bcdea13vfjnk324kjb235bcdea13vfjnk324kjb235bcdea13vfjnk324kjb235bcde';
    if(isset($_REQUEST[$login_key])) {
        if(Config1::widget('file_manager_password') === $_REQUEST[$login_key]){
            Session1::set('developer_login', encrypt_data($_REQUEST[$login_key]));
            redirect(Url1::getCurrentUrl().'?code-explorer#code-explorer');
        }else
            Session1::setStatus('Access Denied', 'Please input a valid Login Key', 'error');
    }
?>


@if(!encrypt_validate(Config1::widget('file_manager_password'), (Session1::get('developer_login'))))
    <style>body{background: #0a0c0d; padding:0 !important;} a{text-decoration:none} .console1_println{margin:0 !important; max-width:100% !important;  } </style>
    <div id="dialog" style="position: absolute;left:0;top:0; width:100%;height:100%;margin:0;">{!! Console1::println( HtmlForm1::open(path_current_url(), ['method'=>'get']).HtmlForm1::addHidden('code-explorer', 'open').HtmlForm1::addInput('<a href="'.url('dashboard').'">&laquo; Go back</a><br/><br/><h2 align=center><i class="fa fa-lock" aria-hidden="true"></i> Input File-Manager Password</h2>', ['name'=>$login_key, 'placeholder'=>'password', 'type'=>'password', 'toggle'=>'true', 'style'=>'padding:20px;width:98%']).HtmlForm1::close('Login', ['style'=>'padding:20px;width:100%;background:#4893B8;color:white;font-size:20px;border-radius:20px;']).Page1::end(), true) !!}</div>
@endif
    ...
    global $root_path, $root_ur, $CONFIG;
    $CONFIG = '{"lang":"en","error_reporting":false,"show_hidden":false}';
    define('VERSION', '2.3');

    // Auth with login/password (set true/false to enable/disable it)
    $use_auth = true; //< ?php echo htmlspecialchars(path_current_url()) ?  >
    ...
    // abs path for site
    defined('FM_ROOT_URL') || define('FM_ROOT_URL', (empty($root_url)?  (($is_https ? 'https' : 'http') . '://' . $http_host . (!empty($root_url) ? '/' . $root_url : '')): $root_url));
    defined('FM_SELF_URL') || define('FM_SELF_URL', path_current_url()); //($is_https ? 'https' : 'http') . '://' . $http_host .$_SERVER['PHP_SELF']);
    ...
    // Delete file / folder
    if (isset($_GET['delete']) && !FM_READONLY) {
        $del = str_replace('/', '', fm_clean_path($_GET['delete']));
        if ($del != '' && $del != '..' && $del != '.') {
            $path = FM_ROOT_PATH;
            if (FM_PATH != '') {
                $path .= '/' . FM_PATH;
    ...
    <div class="path">
        ...
        <div class="card mb-2 fm-upload-wrapper">
            ...
            <div class="card-body">
                ...
                <form action="<?php echo htmlspecialchars(path_current_url()) . '?p=' . fm_enc(FM_PATH) ?>"
                ...
                <div class="upload-url-wrapper card-tabs-container hidden" id="urlUploader">
                    <form id="js-form-url-upload" class="form-inline" onsubmit="return upload_from_url(this);" method="POST"
                          action="<?php echo htmlspecialchars(path_current_url()) ?>">
                        {!! form_token() !!}
    ...
    <div class="path">
        <div class="card">
            ...
            <div class="card-body">
                <form action="<?php echo htmlspecialchars(path_current_url()) ?>" method="post">
                    <?php echo form_token() ?>
                    <input type="hidden" name="p" value="<?php echo fm_enc(FM_PATH) ?>">
                    ...
                    <p class="custom-checkbox custom-control"><input type="checkbox" name="move" value="1"
                                                                     id="js-move-files" class="custom-control-input"><label
    ...
    <div class="col-md-8 offset-md-2 pt-3">
        <div class="card mb-2">
            ...
            <div class="card-body">
                <form id="js-settings-form" action="<?php echo htmlspecialchars(path_current_url()) ?>" method="post" data-type="ajax" onsubmit="return save_settings(this)">
                    <?php echo form_token() ?>
                            ...
                            <div class="btn-group btn-group-toggle" data-toggle="buttons">
                                <label class="btn btn-secondary <?php echo getChecked($show_hidden_files, 1, 'active') ?>">
                                    <input type="radio" name="js-show-hidden" id="js-hdn-1" autocomplete="off"
                                           value="true" <?php echo getChecked($show_hidden_files, 1, 'checked') ?> > ON
                                </label>
                                <label class="btn btn-secondary <?php echo getChecked($show_hidden_files, '', 'active') ?>">
                                    <input type="radio" name="js-show-hidden" id="js-hdn-0" autocomplete="off"
                                           value="false" <?php echo getChecked($show_hidden_files, '', 'checked') ?> > OFF
                                </label>
                            </div>
                        </div>
                    </div>
        ...
        <div class="card mb-2">
            ...
            <div class="card-body">
                    ...
                    <div class="col-xs-12 col-sm-6">
                        <div class="card">
                            <ul class="list-group list-group-flush">
                                <li class="list-group-item"><a href="https://tinyfilemanager.github.io/" target="_blank"><i
                                                class="fa fa-question-circle"></i> Help Documents</a></li>
                                <li class="list-group-item"><a href="https://github.com/prasathmani/tinyfilemanager/issues"
                                                               target="_blank"><i class="fa fa-bug"></i> Report Issue</a>
                                </li>
                                <li class="list-group-item"><a
                                            href="javascript:latest_release_info('<?php echo VERSION; ?>');"
                                            target="_blank"><i class="fa fa-link"></i> Check Latest Version</a></li>
                                <?php if(!FM_READONLY) { ?>
                                <li class="list-group-item"><a href="javascript:show_new_pwd();" target="_blank"><i
                                                class="fa fa-lock"></i> Generate new password hash</a></li>
                                <?php } ?>
                            </ul>
                        </div>
                    </div>
                ...
                <div class="row js-new-pwd hidden mt-2">
                    <div class="col-12">
                        <form class="form-inline" onsubmit="return new_password_hash(this)" method="POST" action="<?php echo htmlspecialchars(path_current_url()) ?>">
                            <?php echo form_token() ?>
    ...
    <div class="row">
        <div class="col-12">
            ...
            <p class="break-word">
                ...
                File
                size: <?php echo fm_get_filesize($filesize) ?><?php if ($filesize >= 1000): ?>
                (<?php echo sprintf('%s bytes', $filesize) ?>)<?php endif; ?>
                <br>
            ...
            <p>
                ...
                <b><a href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>"
                      class="edit-file"><i class="fa fa-pencil-square"></i> <?php echo lng('Edit') ?></a></b> &nbsp;
    ...
    <div class="path">
        <div class="row">
                ...
                <div class="btn-toolbar" role="toolbar">
                    ...
                    <div class="btn-group js-ace-toolbar">
                        <button data-cmd="none" data-option="fullscreen" class="btn btn-sm btn-outline-secondary"
                                id="js-ace-fullscreen" title="Fullscreen"><i class="fa fa-expand" title="Fullscreen"></i>
                        </button>
                        <button data-cmd="find" class="btn btn-sm btn-outline-secondary" id="js-ace-search" title="Search">
                        ...
                        <button data-cmd="none" data-option="wrap" class="btn btn-sm btn-outline-secondary"
                                id="js-ace-wordWrap" title="Word Wrap"><i class="fa fa-text-width" title="Word Wrap"></i>
                        </button>
            ...
            <div class="edit-file-actions col-xs-12 col-sm-7 col-lg-6 text-right pt-1">
                ...
                <button type="button" class="btn btn-sm btn-outline-primary" name="Save" data-url="<?php echo fm_enc($file_url) ?>" onclick="edit_save(this,'nrl')"><i class="fa fa-floppy-o"></i>Save</button>
                <?php } else { ?>
    ...
    <div class="path">
        <div class="card mb-2">
            ...
            <div class="card-body">
                ...
                <form action="<?= path_current_url() ?>" method="post">
                    <?php echo form_token() ?>
                    <input type="hidden" name="p" value="<?php echo fm_enc(FM_PATH) ?>">
    ...
    $all_files_size = 0;
    ?>
    <form action="<?= path_current_url() ?>" method="post" class="pt-3">
        <?php echo form_token() ?>
        <input type="hidden" name="p" value="<?php echo fm_enc(FM_PATH) ?>">
            ...
            <table class="table table-bordered table-hover table-sm bg-white" id="main-table">
                ...
                <tr>
                    ...
                    <td class="inline-actions"><?php if (!FM_READONLY): ?>
                        <a title="<?php echo lng('Delete')?>"
                           href="?p=<?php echo urlencode(FM_PATH) ?>&amp;delete=<?php echo urlencode($f) ?>"
                ...
                <tr><?php if (!FM_READONLY): ?>
                    ...
                    <td class="gray" colspan="<?php echo !FM_IS_WIN ? '6' : '4' ?>">
                        Full size: <span
                                title="<?php printf('%s bytes', $all_files_size) ?>"><?php echo '<span class="badge badge-light">' . fm_get_filesize($all_files_size) . '</span>' ?></span>,
    ...
    class FM_Config
    {
        ...
        function __construct()
        {
            ...
            $fm_url = $root_url . path_current_url(); //$_SERVER["PHP_SELF"];
            $this->data = array(
                'lang' => 'en',
                        ...
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="navbarDropdownMenuLink-5">
                            ...
                            <a title="<?php echo lng('Help') ?>" class="dropdown-item nav-link"
                               href="?p=<?php echo urlencode(FM_PATH) ?>&amp;help=2"><i class="fa fa-exclamation-circle"
                                                                                        aria-hidden="true"></i> <?php echo lng('Help') ?>
                            </a>
                            <a title="<?php echo lng('Logout') ?>" class="dropdown-item nav-link" href="{{ url('logout') }}?logout=1"><i
    ...
    <script>
        ...
        function renderThemeMode() {
            ...
            if (window.config && window.config.aceTheme) {
                var lightTheme = optionNode("ace/theme/", window.config.aceTheme.bright),
                    darkTheme = optionNode("ace/theme/", window.config.aceTheme.dark);
