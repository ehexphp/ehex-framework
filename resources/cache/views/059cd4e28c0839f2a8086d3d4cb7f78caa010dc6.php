    ...
    <div class="main-content-container container-fluid px-4">
        <div class="page-header row no-gutters py-4">
            <div class="col-12 col-sm-4 text-center text-sm-left mb-0">
                ...
                <h3 class="page-title"> Message <?php echo Inbox::getIfModerator($userInfo->id)? "<span class='badge badge-primary'>".Inbox::getIfModerator($userInfo->id)." Department </span>": ""; ?></h3>
            </div>
        </div>



        <?php if(isset($_REQUEST["track_id"]) && !empty($_REQUEST['track_id'])): ?>
            <?php
                // get track_id
                ...
                <div class="col-md-7 mb-4 mx-auto">
                    ...
                    <div class="card card-small">
                        <div class='card-header border-bottom'>
                            <strong class="float-left"><i class='fa fa-user' aria-hidden='true'></i> <?php echo $partnerInfo['full_name']; ?></strong>
                            <a class="float-right text-danger" onclick="Popup1.confirmLink('Delete Message', 'Are you sure you want to delete this conversation?', '<?php echo e(Url1::actionLink(Inbox::class, "deleteConversationForUser(".String1::replaceMany($track_id, ',', '--').")")); ?>')"><i class="fa fa-trash" aria-hidden="true"></i> Delete Message</a>
                        </div>

                        <div class='card-body jumbotron chatBox' id="chatBox">
                            <?php if($totalConversation > 0): ?>
                                <?php $__currentLoopData = $allPreviousConversation; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $chat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <?php $isMeTheSender = ($chat['user_id'] == $userInfo->id); ?>
                                    <div class='chat <?php echo e(($isMeTheSender? "request": "response")); ?>'>
                                        <div>
                                            <small class="text-white-50"><?php echo ($isMeTheSender? "": User::getField($chat['user_id'], 'user_name', "#$chat[user_id]"))."<br/>"; ?></small>
                                            <strong><?php echo e($chat->subject); ?></strong>
                                            <?php echo e($chat->message); ?>

                                        </div>
                                        <div class='mt-1'>
                                            <div class='<?php echo e(($isMeTheSender? "pull-right float-right": "")); ?>'>
                                                <?php echo Inbox::renderAttachment($chat->attachment); ?>

                                                <em class="text-white-50" style='font-size:11px;'><?php echo e(DateManager1::diffForHumans($chat->created_at)); ?></em>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <?php else: ?>
                                <div class="text-muted m-5 text-center">
                                    ...
                                    <h5 class="text-muted"><strong>Chat with <?php echo e($partnerInfo['full_name']); ?></strong></h5>
                                </div>
                            <?php endif; ?>
                        ...
                        <div class='card-footer' style='margin-top:-30px;'>
                            <?php echo HtmlForm1::open("Inbox@processSendMessage()").
        ...
        <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 mb-4">
                <div class="card card-small">
                    <div class="card-header border-bottom">
                        <h6 class="m-0"> <?php echo Inbox::getModelClassName(true); ?>

                            <button class="btn btn-danger float-right"  onclick="Popup1.confirmLink('Delete All Messages', 'Are you sure you want to delete all messages?', '<?php echo e(Url1::actionLink(Inbox::class, "deleteAllConversationForUser()")); ?>')"><i class="fa fa-trash" aria-hidden="true"></i> Clear All</button>
                            ...
                            <script>
                                function adminList(){
                                    return Popup1.confirmOption('Select', 'select department to chat with', JSON.parse('<?php echo json_encode(Inbox::$APP_CONFIG['CHAT_DEPARTMENT_USER_ID_LIST']); ?>'), function(id){
                                        Url1.redirect("<?php echo e(url('inbox')); ?>?track_id=" + id + ",<?php echo e($userInfo->id); ?>" )
                                    });
                                }
                            </script>
                        </h6>
                    </div>
                    <div class="card-body pt-0" style="margin-top:10px;"><?php echo Inbox::manage()->render(); ?></div>
                </div>
            </div>
        </div>
    </div>
...
<?php echo $__env->make('layouts.shards_dashboard.template', ['page_title'=>"Inbox", 'allow_xcrud'=>true], array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>